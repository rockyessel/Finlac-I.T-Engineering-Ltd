import { FormErrorProps } from '../components/form';
...
export default function FormValidation(
  ...
  switch (formType) {
    case 'name':
      try {
        const nameValidation = (formValue: string) => {
          currentState = false;
    ...
    case 'email':
      try {
        const emailValidation = (formValue: string) => {
          currentState = false;
          if (!formValue) {
            const formState = {
              ...formErr,
    ...
    case 'country':
      try {
        const countryValidation = (formValue: string) => {
          currentState = false;
          if (!formValue) {
            const formState = {
              ...formErr,
    ...
    case 'ph_number':
      try {
        const ph_numberValidation = (formValue: string) => {
          currentState = false;
          if (!formValue) {
            const formState = {
              ...formErr,
    ...
    case 'message':
      try {
        const messageValidation = (formValue: string) => {
          currentState = false;
          if (!formValue) {
            const formState = {
              ...formErr,
            ...
            const formState = {
              ...formErr,
              state: true,
              msg: 'Characters should be more than 100',
            };
            setFormErr(formState);
